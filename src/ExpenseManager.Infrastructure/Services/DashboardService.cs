...
internal sealed class DashboardService : IDashboardService
{
    ...
    public async Task<GetDashboardOverviewResponse> GetOverviewAsync(GetDashboardOverviewRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var monthReceiptsQuery = _dbContext.Receipts
            .AsNoTracking()
            .Where(receipt => receipt.UserId == request.UserId && receipt.ReceiptDate >= monthStart && receipt.ReceiptDate <= monthEnd);

        var currentMonthExpenses = await monthExpensesQuery
            .SumAsync(expense => (decimal?)expense.Amount.Amount, cancellationToken) ?? 0m;

        var currentMonthCommission = await monthReceiptsQuery
            .SumAsync(receipt => receipt.NetAmount.Amount * (receipt.CommissionPercent ?? 0m) / 100m, cancellationToken);
        ...
        var currentMonthProfit = currentMonthCommission - currentMonthExpenses;
    ...
    {
    var summary = new DashboardSummary(0, 0, 0, 0, 0, currency);
