...
using ExpenseStatusOptionViewModel = ExpenseManager.Desktop.ViewModels.Items.ExpenseStatusOptionViewModel;
...
public sealed partial class ExpensesViewModel : ViewModelBase, ILoadableViewModel, ILocalizableViewModel
{
    ...
    private void ApplyExpenses(IEnumerable<ExpenseListItem> items)
    {
        ...
        {
            var amountDisplay = item.Amount.ToString("N2", culture);
            var currencyDisplay = item.Currency.ToString().ToUpperInvariant();
    ...
    private async Task LoadExpenseDetailsAsync(Guid expenseId, CancellationToken cancellationToken)
    {
        ...
        var culture = CultureInfo.CurrentCulture;
        var amountDisplay = details.Amount.ToString("N2", culture);
        var currencyDisplay = details.Currency.ToString().ToUpperInvariant();
    ...
    public sealed partial class ExpenseFormViewModel : ObservableObject
    {
        ...
        public ObservableCollection<ExpenseStatusOptionViewModel> StatusOptions { get; } = new();
        ...
        public void BeginEdit(ExpenseDetails details, IReadOnlyCollection<CategoryOptionViewModel> categories)
        {
            ...
            AmountInput = details.Amount.ToString("0.##", CultureInfo.CurrentCulture);
        ...
        public CreateExpenseRequest ToCreateRequest(Guid userId)
        {
            ...
            if (!TryParseDecimal(AmountInput, out var amount))
            {
                throw new InvalidOperationException(Translate("ERROR_INVALID_AMOUNT"));
            }
        ...
        public void RefreshStatusOptions() => PopulateStatuses();
        ...
        {
            var key = BuildPaymentMethodKey(method);
        ...
        {
            var key = BuildExpenseStatusKey(status);
            var value = _localization.GetString(key);
            return string.Equals(value, key, StringComparison.Ordinal) ? status.ToString() : value;
        }

        private void PopulateStatuses()
        {
            var previousSelection = SelectedStatus;

            StatusOptions.Clear();
            foreach (var status in Enum.GetValues<ExpenseStatus>())
            {
                var displayName = TranslateExpenseStatus(status);
                StatusOptions.Add(new ExpenseStatusOptionViewModel(status, displayName));
            }
            ...
            if (StatusOptions.Any(option => option.Value == previousSelection))
            {
                SelectedStatus = previousSelection;
            }
            else
            {
    ...
    private static string BuildExpenseStatusKey(ExpenseStatus status)
    {
        return $"EXPENSE_STATUS_{ToUpperUnderscore(status.ToString())}";
    }

    private static string BuildPaymentMethodKey(PaymentMethod method)
    {
        return $"PAYMENT_METHOD_{ToUpperUnderscore(method.ToString())}";
    }
